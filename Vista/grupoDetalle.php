...
<main class="mx-auto max-w-6xl px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <div class="flex gap-2">
            <a href="?menu-item=Grupos&action=editar&id=<?php echo htmlspecialchars($grupo['id']); ?>"
...
<div id="modalCambiarRol" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Cambiar Rol de Miembro</h3>
            <form action="?menu-item=Grupos&action=actualizar_rol" method="POST">
                <input type="hidden" name="grupo_id" value="<?php echo htmlspecialchars($grupo['id']); ?>">
...
<script>
    ...
    function eliminarMiembro(usuarioId, nombreUsuario) {
        if (confirm(`¿Estás seguro de que deseas eliminar a "${nombreUsuario}" del grupo?`)) {
            window.location.href = `?menu-item=Grupos&action=eliminar_miembro&grupo_id=<?php echo $grupo['id']; ?>&usuario_id=${usuarioId}`;
        }
    }
    ...
    function confirmarEliminacion() {
        if (confirm('⚠️ ¿Estás seguro de que deseas eliminar este grupo?\n\nEsta acción eliminará el grupo y todos sus miembros de forma permanente.')) {
            ...
            form.method = 'POST';
            form.action = '?menu-item=Grupos&action=eliminar&id=<?php echo $grupo['id']; ?>';
