        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            <?php if (!empty($noticias)): ?>
                <?php foreach ($noticias as $index => $noticiaItem): ?>
                    <article class="bg-white rounded-3xl shadow-xl overflow-hidden transform hover:scale-105 hover:rotate-1 transition-all duration-500 group" style="animation: fadeInUp 0.6s ease-out <?= $index * 0.1 ?>s both">
                        <div class="relative overflow-hidden h-64">
                            <?php if (!empty($noticiaItem['imagen']) && file_exists($noticiaItem['imagen'])): ?>
                                <img src="<?= htmlspecialchars($noticiaItem['imagen']) ?>" alt="<?= htmlspecialchars($noticiaItem['titulo']) ?>" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                            <?php else: ?>
            ...
            <div class="bg-white rounded-3xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto transform scale-95 transition-all duration-300" id="modalContent">
                ...
                <form id="noticiaForm" action="#" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" id="titulo" name="titulo" required class="w-full px-6 py-4 text-lg border-2 border-gray-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-[#D0B8A8] focus:border-[#D0B8A8] transition-all duration-300">
                            </div>
                            <div>
                                <label for="imagen" class="block text-lg font-bold text-gray-900 mb-3">Imagen Principal</label>
                                <input type="file" id="imagen" name="imagen" accept="image/*" class="w-full px-6 py-4 text-lg border-2 border-dashed border-gray-300 rounded-2xl">
                                <p class="text-sm text-gray-500 mt-2">JPG, PNG o GIF. Máximo 5MB</p>
                            </div>
                        </div>
                        <div class="flex items-center justify-center">
                            <img id="imagenPreview" src="" alt="Vista previa" class="max-w-full h-48 object-cover rounded-2xl shadow-lg hidden">
                    ...
                    <div>
                        ...
                        <textarea id="descripcion" name="descripcion" rows="6" required class="w-full px-6 py-4 text-lg border-2 border-gray-200 rounded-2xl resize-none"></textarea>
