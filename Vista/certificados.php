            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        // ÚNICO BOTÓN: Genera desde cero y muestra el PDF
                    if (data.estado !== 'pendiente_pago') {
                        let downloadUrl = "<?= url('certificados/descargar') ?>";
                        let separator = downloadUrl.includes('?') ? '&' : '?';
                        html += `<a href="${downloadUrl}${separator}id=${data.id}"
                                    target="_blank"
                                    class="px-3 py-1.5 bg-[#D0B8A8] hover:bg-[#ab876f] text-white rounded text-sm font-medium transition flex items-center justify-center gap-1"
