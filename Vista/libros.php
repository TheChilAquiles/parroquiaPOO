    ...
    <div class="grid grid-cols-5 bg-white border  border-gray-200  w-full h-full p-4 rounded-md gap-2">
        ...
        <?php if (isset($cantidad) && $cantidad > 0): ?>
            ...
            <?php for ($i = 1; $i <= $cantidad; $i++): ?>
                ...
                <a href="<?= url('sacramentos/libro') ?>?tipo=<?= $tipoId ?>&numero=<?= $i ?>" class="w-full cursor-pointer relative flex justify-center items-center p-2 rounded-md border border-gray-500/50 hover:bg-emerald-100 group">
        ...
        <a href="<?= url('libros/crear') ?>?tipo=<?= $tipoId ?>" class="w-full cursor-pointer relative flex justify-center items-center p-2 rounded-md border border-gray-500/50 hover:bg-emerald-100 group">
            <svg class="w-60" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
